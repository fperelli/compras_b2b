...
@router.post("/webhooks/{channel}", tags=["Integraciones"])
async def handle_webhook(
    ...
    """
    Endpoint universal para recibir webhooks de diferentes canales.
    """
    from src.integrations import WhatsAppAdapter, SlackAdapter
    
    service = get_negotiation_service(tenant_id)
    
    if channel == "whatsapp":
        adapter = WhatsAppAdapter(tenant_id)
    elif channel == "slack":
        adapter = SlackAdapter(tenant_id)
    else:
        raise HTTPException(status_code=400, detail="Channel not supported")

    # 1. Parse incoming
    parsed = await adapter.parse_incoming(payload)
    if "error" in parsed:
        return parsed

    # 2. Get AI Response (Treating it as chat for now)
    agent_output = await service.run_chat(
        request_id=f"webhook-{channel}", 
        history=[{"role": "user", "content": parsed["text"]}]
    )

    # 3. Format and Send back
    formatted = await adapter.format_outgoing(agent_output, context=parsed)
    success = await adapter.send_response(formatted)
